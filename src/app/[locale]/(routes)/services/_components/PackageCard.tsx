...
export const PackageCard = ({ package: pkg }: PackageCardProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative h-[180px] bg-linear-to-br from-primary/5 to-[#3B82F6]/5 overflow-hidden">
        <img
      ...
      <div className="p-5 flex flex-col flex-1">
        <h3
          className="text-[17px] md:text-[18px] text-[#111827] mb-3 line-clamp-2 leading-snug"
          ...
          <div className="space-y-1.5">
            {pkg.includedServices.slice(0, 3).map((service, idx) => (
              <div key={idx} className="flex items-start gap-2">
                ...
                <span className="text-[13px] text-[#6B7280] line-clamp-1 leading-tight">
                  {service}
                </span>
              </div>
            ))}
        ...
        <div className="mb-4">
          <p className="text-[12px] text-[#6B7280] mb-1.5">Starting from</p>
          <div className="flex items-baseline flex-wrap gap-2">
            {pkg.originalPrice && pkg.discountPercent ? (
              <>
                <span className="text-base text-[#6B7280] line-through decoration-[#EF4444] decoration-2">
                  {pkg.originalPrice.toLocaleString()} AZN
                </span>
                <div className="bg-[#FEF3C7] px-2 py-1 rounded-md">
                  <span
          ...
          <div className="flex items-baseline gap-1 mt-1">
            ...
            <span className="text-sm text-[#6B7280]">{pkg.priceUnit}</span>
          </div>
        </div>
      ...
      <div className="absolute bottom-0 w-full left-0 right-0 bg-linear-to-t from-primary via-primary/95 to-transparent text-white p-5 translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out flex">
        <div className="space-y-2 w-full">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm">Min Guests: {pkg.minGuests}</span>
          </div>
          <div className="flex items-center gap-2">
            <Calendar className="w-5 h-5" />
            <span className="text-sm">{pkg.advanceBooking}</span>
          </div>
        </div>
        <div className="h-full flex items-center w-full justify-center">
          <MoveRight
            className="w-8 h-8 ml-20 mt-5 text-white animate-[slideX_1.6s_ease-in-out_infinite]"
            aria-hidden
          />
          <style>{`
                        @keyframes slideX {
